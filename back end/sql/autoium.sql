...
CREATE TABLE `api_case` (
	...
	`host` VARCHAR ( 255 ) CHARACTER SET utf8 NOT NULL COMMENT '协议和域名',
	`path` VARCHAR ( 255 ) CHARACTER SET utf8 COMMENT '路径',
	`req_method` INT ( 2 ) NOT NULL DEFAULT '0' COMMENT '请求类型;0:GET;1:PUT;2:PUT;3:DELETE',
	`req_header` VARCHAR ( 255 ) COMMENT '用例的请求头',
	`req_params` VARCHAR ( 255 ) COMMENT '用例的请求参数',
	`req_body_type` INT ( 2 ) COMMENT '请求body的类型;0:json;1:form;2:file',
	`req_body_json` text COMMENT '用例的json类型请求体',
	`req_body_form` text COMMENT '用例的form类型请求体',
	`req_body_file` VARCHAR ( 255 ) CHARACTER SET utf8 COMMENT '用例的文件类型请求体,用阿里OSS存储',
	`examine` text COMMENT '请求的断言',
	`pre_action` text COMMENT '前置操作例如登录,是所有接口用例中的一个',
	`description` VARCHAR ( 255 ) COMMENT '用例的描述',
